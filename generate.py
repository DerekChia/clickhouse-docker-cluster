...
def generate_config(args, context):
    ...
    if args.keeper_mode == "embedded":
        ...
        # create configs directory using hostname
        ...
        Path(f"{args.directory}/configs/prometheus").mkdir(parents=True, exist_ok=True)

        # create grafana directory
        Path(f"{args.directory}/configs/grafana").mkdir(parents=True, exist_ok=True)
        Path(f"{args.directory}/configs/grafana/dashboards").mkdir(
            parents=True, exist_ok=True
        )
        ...
        # [all] prometheus.xml
        template = environment.get_template("prometheus_enable.txt")

        for count, hostname in enumerate(all_hostnames):
            filename_generated = (
                f"{args.directory}/configs/{hostname}/prometheus_enable.xml"
            )
            content = template.render(
                hostnames=all_hostnames,
                cluster_name=args.cluster_name,
        ...
        template = environment.get_template("prometheus/prometheus.txt")

        # [grafana]
        # datasource.yml, dashboard.yml
        # dashboards/{cluster-analysis,data-analysis,query-analysis}-{HOSTNAME}.json
